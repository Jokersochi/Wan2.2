...
## 2025-02-18 - Graceful CLI Error Handling
**Learning:** Raw assert statements in Python CLI scripts using argparse lead to intimidating stack traces for users.
**Action:** Replace `assert` validation with `parser.error()` inside `argparse` driven scripts to provide clean, usage-aware error messages and standard exit codes.
