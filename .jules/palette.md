...
**Learning:** For Python CLI applications using `argparse`, 'Palette' UX improvements include replacing raw `assert` statements used for argument validation with `parser.error()`. This provides clean, usage-aware error messages and standard exit codes, avoiding intimidating stack traces for users.
**Action:** When validating arguments in a CLI tool, always pass the parser to the validation function and use `parser.error()` rather than `assert` or `raise ValueError` if possible.
