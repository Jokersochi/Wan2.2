...
def _validate_args(args, parser=None):
    ...
    if args.ckpt_dir is None:
        raise_error("Please specify the checkpoint directory.")
    if args.task not in WAN_CONFIGS:
        raise_error(f"Unsupport task: {args.task}")
    ...
    if args.task == "i2v-A14B":
        if args.image is None:
            raise_error("Please specify the image path for i2v.")
    ...
    if not 's2v' in args.task:
        if args.size not in SUPPORTED_SIZES[args.task]:
            raise_error(f"Unsupport size {args.size} for task {args.task}, supported sizes are: {', '.join(SUPPORTED_SIZES[args.task])}")
