...
def _validate_args(args, parser=None):
    ...
    if args.ckpt_dir is None:
        error_exit("Please specify the checkpoint directory.")
    if args.task not in WAN_CONFIGS:
        error_exit(f"Unsupport task: {args.task}")
    ...
    if args.task == "i2v-A14B":
        if args.image is None:
            error_exit("Please specify the image path for i2v.")
    ...
    if not 's2v' in args.task:
        if args.size not in SUPPORTED_SIZES[args.task]:
            error_exit(f"Unsupport size {args.size} for task {args.task}, supported sizes are: {', '.join(SUPPORTED_SIZES[args.task])}")
...
def generate(args):
    ...
    if world_size > 1:
        ...
        if args.ulysses_size > 1:
            raise ValueError("sequence parallel are not supported in non-distributed environments.")

    if args.ulysses_size > 1:
        if args.ulysses_size != world_size:
            raise ValueError("The number of ulysses_size should be equal to the world size.")
