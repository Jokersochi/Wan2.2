...
def generate(args):
    ...
    if world_size > 1:
        ...
        if args.t5_fsdp or args.dit_fsdp:
            raise ValueError(f"t5_fsdp and dit_fsdp are not supported in non-distributed environments.")
        if args.ulysses_size > 1:
            raise ValueError(f"sequence parallel are not supported in non-distributed environments.")

    if args.ulysses_size > 1:
        if args.ulysses_size != world_size:
            raise ValueError(f"The number of ulysses_size should be equal to the world size.")
