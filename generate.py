...
def _validate_args(args, parser):
    ...
    if args.ckpt_dir is None:
        parser.error("Please specify the checkpoint directory using --ckpt_dir.")
    if args.task not in WAN_CONFIGS or args.task not in EXAMPLE_PROMPT:
        parser.error(f"Unsupported task: {args.task}. Supported tasks are: {', '.join(WAN_CONFIGS.keys())}")
    ...
    if args.task == "i2v-A14B":
        if args.image is None:
            parser.error("Please specify the image path for i2v using --image.")
    ...
    if not 's2v' in args.task:
        if args.size not in SUPPORTED_SIZES[args.task]:
            parser.error(f"Unsupported size {args.size} for task {args.task}, supported sizes are: {', '.join(SUPPORTED_SIZES[args.task])}")
