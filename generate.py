...
def _validate_args(args, parser=None):
    ...
    if args.ckpt_dir is None:
        if parser: parser.error("Please specify the checkpoint directory.")
        else: raise ValueError("Please specify the checkpoint directory.")
    if args.task not in WAN_CONFIGS:
        if parser: parser.error(f"Unsupport task: {args.task}")
        else: raise ValueError(f"Unsupport task: {args.task}")
    ...
    if args.task == "i2v-A14B":
        if args.image is None:
            if parser: parser.error("Please specify the image path for i2v.")
            else: raise ValueError("Please specify the image path for i2v.")
    ...
    if not 's2v' in args.task:
        if args.size not in SUPPORTED_SIZES[args.task]:
            msg = f"Unsupport size {args.size} for task {args.task}, supported sizes are: {', '.join(SUPPORTED_SIZES[args.task])}"
